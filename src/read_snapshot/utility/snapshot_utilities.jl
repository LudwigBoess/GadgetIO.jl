...
"""
function check_blocksize(f::IOStream, position_before::Integer, blocksize_before::Integer; swap::Bool = false)

    seek(f,position_before+blocksize_before+12)
    blocksize_after = read(f,UInt32)
    if swap 
        blocksize_after = bswap(blocksize_after)
    end 
    ...
    if blocksize_before == blocksize_after
        ...
        for N_check = 1:10
            ...
            blocksize_after_fix = read(f,UInt32) 
            if swap
                blocksize_after_fix = bswap(blocksize_after_fix)
            end  
            blocksize_after_fix = blocksize_after_fix + N_check*4294967296
...
Checks for integer overflow in the size of the block.
"""
function check_blocksize_format1(f::IOStream, position_before::Integer, blocksize_before::Integer; swap::Bool = false)
